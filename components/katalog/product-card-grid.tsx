                        ...
                        {paginated.map((battery, i) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className="flex items-center gap-3 px-3 py-2.5 bg-card hover:bg-muted/30 transition-colors duration-200"
                                >
                                    {/* Thumbnail */}
                                    <div className="relative w-12 h-12 shrink-0 rounded-lg overflow-hidden border border-border/40 bg-muted/30">
                                        {battery.gambar ? (
                                            <Image
                                                src={battery.gambar}
                                                alt={battery.nama}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-sm font-semibold text-foreground leading-tight truncate">{battery.nama}</p>
                                        <div className="flex items-center gap-2 mt-0.5">
                                            <p className="text-xs font-bold text-foreground">{formatRupiah(battery.harga_jual)}</p>
                                            <span className={`text-[10px] font-bold ${getStokStyle(battery.stok)}`}>
                                                {battery.stok} unit
                                            </span>
                                        </div>
                                    </div>

                                    {/* Badges (hidden on xs, shown on sm) */}
                                    <div className="hidden sm:flex flex-col gap-1 shrink-0">
                                        ...
                                        {spec?.kapasitas && (
                                            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold border border-border/60 bg-muted/50 text-muted-foreground text-center">
                                                {spec.kapasitas}Ah
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex gap-1 shrink-0">
                                        <button
                                            className="w-7 h-7 rounded-lg border border-border/60 flex items-center justify-center text-muted-foreground hover:text-primary hover:border-primary/50 transition-all duration-150 active:scale-[0.95]"
                                        ...
                                        <button
                                            className="w-7 h-7 rounded-lg border border-border/60 flex items-center justify-center text-muted-foreground hover:text-destructive hover:border-destructive/40 transition-all duration-150 active:scale-[0.95]"
